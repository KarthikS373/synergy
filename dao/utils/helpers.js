...
module.exports.setupProof = async function (context, _index = 4) {
  ...
  const allowlistAddresses = [deployer].concat(
    accounts.filter((_, idx) => idx < _index)
  );
  ...
  const deps = {
    ...
    proofs: allowlistAddresses.map((addr) =>
      merkleTree.getHexProof(keccak256(addr))
    ),
    badProof: merkleTree.getHexProof(keccak256(accounts[_index])),
    allowlistAddresses,
    allowlistAccounts: await Promise.all(
      allowlistAddresses.map((v) => ethers.getSigner(v))
    ),
    accounts,
...
module.exports.membershipMintAndDelegate = async function (
  membership,
  context
) {
