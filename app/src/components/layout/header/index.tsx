  ...
  return (
    <>
      <nav className="bg-white relative flex items-center justify-between mx-8 px-4 py-4">
        ...
        <ul className="absolute left-1/2 top-1/2 hidden -translate-x-1/2 -translate-y-1/2 transform lg:mx-auto lg:flex lg:w-auto lg:items-center lg:space-x-6">
          {navLinks.map((link, index) => {
            return (
              <>
                <li>
                  <Link className="text-sm text-gray-400 hover:text-gray-500" href={link.path}>
                    {link.title}
                  </Link>
                </li>
        ...
        <a
          className="text-white hidden rounded-xl bg-blue-500 px-6 py-2 text-sm font-bold transition duration-200 hover:bg-blue-600 lg:inline-block"
          href="#"
        >
          Sign up
        </a>
      ...
      <div className="relative z-50">
        <div className={clsx("fixed inset-0 -z-10 bg-gray-800 opacity-25", show ? "" : "hidden")} />
        <nav
          className={clsx(
            "fixed bottom-0 top-0 z-10 flex w-5/6 max-w-sm flex-col overflow-y-auto border-r bg-slate-200 px-6 py-6 transition-all",
            show ? "left-0" : "-left-96"
          )}
        >
          ...
          <div className="mt-auto">
            <div className="flex w-full gap-4 pt-6">
              <Button className="mb-3 block grow rounded-xl bg-gray-50 px-4 py-3 text-center text-xs font-light leading-loose hover:bg-gray-100">
                Sign in
              </Button>
              <Button className="text-white mb-2 block grow rounded-xl bg-blue-600 px-4 py-3 text-center text-xs font-light leading-loose  hover:bg-blue-700">
                Sign Up
              </Button>
            </div>
